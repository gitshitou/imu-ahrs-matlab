...
MPU9150.SENSgyro(:,1)= MPU9150.SENSgyro(:,2);
MPU9150.SENSacc(:,1) =MPU9150.SENSacc(:,2);

my_magTEST = zeros(3,length(MPU9150.MATLABtime));
% Filtering
...
%     MPU9150.quat(i,:) = MPU9150.AHRS.Quaternion;
     w = MPU9150.SENSgyro(:,i)';
     a = MPU9150.SENSacc(:,i)';

     y = ekf_6dof(w(1),w(2),w(3),a(1),a(2),a(3),0.01 ,MPU9150.quat');
    %y = test_ekf(w(1),w(2),w(3),a(1),a(2),a(3),deltat );
%     allX(k,:) = y'; 
    MPU9150.quat(i,:) = y'; 
...
%%测试一下求出来的magNED
% xx = length(MPU9150.MATLABtime);
% plot(xx,my_magTEST(1,:));
...
hold off; i=i+1; subplot(3,1,i); grid on ;hold on; ylabel ('pitch (?')
plot (SENTRAL.error(:,i),'g'); 
hold off; i=i+1; subplot(3,1,i); grid on; hold on; ylabel ('yaw (?'); xlabel ('sample')
...
%%
% for i = 1:length(MPU9150.MATLABtime)
%      my_magTEST(:,i) = Quat2Tbn(MPU9150.quat(:,i))*MPU9150.SENSmag(:,i);
% end